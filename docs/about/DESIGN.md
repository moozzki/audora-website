...
*   **Secondary (Periwinkle):** Used for "Support Moments"—active states of chips or subtle highlights that don't need the intensity of the primary blue. *Note: The document specifies #CACAFD for secondary-container, which aligns with the desired secondary color.*
...
*   **Do** use `tertiary_color_hex` (#E1E0FF) for background accents behind 3D icons to make the blue tones "pop."
...
*   **Don't** use `DEFAULT` roundedness (0.25rem) for small containers. Reserve it for small UI primitives like checkboxes. Large containers must use `xl` (0.75rem).
